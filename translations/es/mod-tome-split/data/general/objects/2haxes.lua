-- mod-tome/data/general/objects/2haxes.lua
-- Total: 14 cadenas
-- Estado: ⏳ Pendiente
-- Ultima actualizacion: --
-- Traductor: --

t("weapon", "arma", "entity type")
t("battleaxe", "hacha de batalla", "entity subtype")
t("axe", "hacha", "entity combat talented")
t("Massive two-handed battleaxes.", "Massive de dos manos.", "_t")
t("iron battleaxe", "hacha de batalla de hierro", "entity name")
t("iron", "hierro", "entity short_name")
t("steel battleaxe", "hacha de batalla de acero", "entity name")
t("steel", "acero", "entity short_name")
t("dwarven-steel battleaxe", "enano de estela batalla hacha", "entity name")
t("d.steel", "ac.dw", "entity short_name")
t("stralite battleaxe", "estralite battleaxe", "entity name")
t("stralite", "estralita", "entity short_name")
t("voratun battleaxe", "voratún hacha de batalla", "entity name")
t("voratun", "voratún", "entity short_name")

